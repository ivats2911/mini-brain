...
export function ThoughtCard({ thought, rule, rules, onDelete, onEdit, onReassign }: Props) {
  ...
  return (
    <li
      // backdrop-blur gives every card its own stacking context, so the picker
      // dropdown would paint under the next card — lift this card while open.
      className={`group rounded-xl border border-white/[0.06] bg-white/[0.03] px-4 py-3 backdrop-blur transition-colors hover:bg-white/[0.05] ${
